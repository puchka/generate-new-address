...
fn get_new_address(pub_key: String) -> Result<AddressInfo, bdk::Error> {
    let mut external_descriptor = "wpkh(".to_owned();
    external_descriptor.push_str(pub_key.as_str());
    external_descriptor.push_str("/84'/0'/0'/0/*)");
    let mut internal_descriptor = String::from("wpkh(").to_owned();
    internal_descriptor.push_str(pub_key.as_str());
...
pub extern "system" fn Java_GenerateNewAddress_getNewAddress(env: JNIEnv,
                                                             ...
                                                             -> jstring {
    let desc: String =
        env.get_string(pub_key).expect("Couldn't get java string!").into();
